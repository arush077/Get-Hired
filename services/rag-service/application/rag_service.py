...
class RAGService:
    ...
    async def _get_embeddings(self, texts: list[str], task: str = "retrieval.passage") -> list[list[float]]:
    ...
    async def ingest_documents(self, resume_text: str, jd_text: str, interview_id: str | None = None) -> dict:
        ...
        for doc_type, text in [("resume", resume_text), ("job_description", jd_text)]:
            ...
            embeddings = await self._get_embeddings(raw_chunks, task="retrieval.passage")
    ...
    async def retrieve(
        ...
        query_embedding = (await self._get_embeddings([query], task="retrieval.query"))[0]
