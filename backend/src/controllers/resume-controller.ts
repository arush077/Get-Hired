...
export async function createResume(req: Request, res: Response): Promise<void> {
  try {
    ...
    const resume = await resumeService.createResume(userId, parsed.data as Record<string, unknown>);
...
${rawText}`;
    ...
    try {
      ...
      getLogger().error({ error: (e as Error).message }, "[IMPORT] LLM parsing failed");
      res.status(500).json({ detail: "Failed to parse resume content" });
      return;
    }

    const resumeData = {
      title: (data.title as string) || file.originalname.split(".")[0],
...
export async function analyzeResume(req: Request, res: Response): Promise<void> {
  try {
    ...
    try {
      ...
      getLogger().error({ error: (e as Error).message }, "[ANALYZE] LLM call failed");
      res.status(500).json({ detail: `AI service error: ${(e as Error).constructor.name}` });
      return;
    }
...
function buildAnalysisPrompt(resume: Record<string, unknown>): string {
...
function resumeToResponse(resume: import("../models/resume.js").Resume): Record<string, unknown> {
  return {
    id: resume.id,
    title: resume.title,
