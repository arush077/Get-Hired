...
const stripeDurations = stripes.map(() => 5 + Math.random() * 3);

export function AnimatedLines({ speed = 1 }: { speed?: number }) {
  return (
    <div className="pointer-events-none absolute inset-0 flex items-end justify-center overflow-hidden">
      {stripes.map((stripe, i) => {
        ...
        const duration = stripeDurations[i] / speed;
        const delay = (i * 0.2) / speed;
