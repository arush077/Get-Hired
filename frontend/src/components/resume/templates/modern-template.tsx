...
export function ModernTemplate({ resume, sectionOrder }: TemplateProps) {
  ...
  return (
    <div className="p-8 bg-white text-neutral-900 text-[13px] leading-relaxed">
      ...
      <div className="text-center mb-5">
        <h1 className="text-2xl font-light tracking-wide">{info.fullName || "Your Name"}</h1>
        <div className="mt-2 text-[12px] text-neutral-500 flex flex-wrap justify-center gap-x-3 gap-y-0.5">
      ...
      <div className="w-16 h-0.5 bg-[#d9c59a] mx-auto mb-5" />
          ...
          return (
            <div key={key} className="mb-5">
              <div className="flex items-center gap-2 mb-3">
                <div className="w-1 h-4 bg-[#d9c59a] rounded-full" />
                <h3 className="text-[11px] font-bold uppercase tracking-[0.15em] text-neutral-700">
                  {SECTION_LABELS[key]}
                </h3>
              </div>
        ...
        if (key === "skills" && resume.skills?.trim()) {
          return (
            <div key={key} className="mb-5">
              ...
              <p className="text-[12px] pl-3">{resume.skills}</p>
            </div>
          );
        }
